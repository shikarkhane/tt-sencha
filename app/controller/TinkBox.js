	...
	getTinkBoxData: function(component) {
                        ...
                        if(!Ext.isEmpty(window.afterTinkSent)) {
                            if(json.groups[i].user == window.contactSelected.data.phone_number) {
                                topItemData.push({
                                    ...
                                    'imgleft': ttapp.util.Common.animationThumbnail(),
                                    'imgright': ttapp.util.Common.animationThumbnail(),
                                    'background': Ext.getStore('phonecontacts').getUserImage(json.groups[i].user) || json.groups[i].default_img,
		...
		var list = Ext.create('Ext.List', {
                    ...
                    '<tpl if="inTime &gt; outTime">',
                        '<div class="list-box" style="background:url({background}) no-repeat 50% 50%">',
                            '<div class="over-lay"></div>',
                                '<span class="inner-detail"><span class="user-name">{user}</span><span class="info"><span class="circle active"><span class="notification-icon"></span><img src={imgleft} ></span><span class="time">{tink_in}</span><span class="tinkinout">Tink in</span></span><span class="info right"><span class="circle"><img src={imgright} ></span><span class="time">{tink_out}</span><span class="tinkinout">Tink out</span></span><span class="arrow"></span></span>',
                        '</div>',
                    '<tpl else>',
                        '<div class="list-box" style="background:url({background}) no-repeat 50% 50%">',
                            '<div class="over-lay"></div>',
                                '<span class="inner-detail"><span class="user-name">{user}</span><span class="info right"><span class="circle active"><span class="notification-icon"></span><img src={imgleft} ></span><span class="time">{tink_in}</span><span class="tinkinout">Tink in</span></span><span class="info"><span class="circle"><img src={imgright} ></span><span class="time">{tink_out}</span><span class="tinkinout">Tink out</span></span><span class="arrow"></span></span>',
                            '</div>',
                    '</tpl>',
                '<tpl else>',
                    '<tpl if="inTime &gt; outTime">',
                        '<div class="list-box" style="background:url({background}) no-repeat 50% 50%">',
                            '<div class="over-lay"></div>',
                                '<span class="inner-detail"><span class="user-name">{user}</span><span class="info"><span class="circle"><img src={imgleft} ></span><span class="time">{tink_in}</span><span class="tinkinout">Tink in</span></span><span class="info right"><span class="circle"><img src={imgright} ></span><span class="time">{tink_out}</span><span class="tinkinout">Tink out</span></span><span class="arrow"></span></span>',
                        '</div>',
                    '<tpl else>',
                        '<div class="list-box" style="background:url({background}) no-repeat 50% 50%">',
                            '<div class="over-lay"></div>',
                                '<span class="inner-detail"><span class="user-name">{user}</span><span class="info right"><span class="circle"><img src={imgleft} ></span><span class="time">{tink_in}</span><span class="tinkinout">Tink in</span></span><span class="info"><span class="circle"><img src={imgright} ></span><span class="time">{tink_out}</span><span class="tinkinout">Tink out</span></span><span class="arrow"></span></span>',
                        '</div>',
                    '</tpl>',
                '</tpl>',
            ],
            store: {
                id: 'tinkBoxStore',
                autoLoad: false,
                fields: ['tink_in', 'tink_out', 'unread', 'user', 'imgleft',  'imgright', 'background', 'number', 'inout', 'inTime', 'outTime' ]
            },
            listeners: {
